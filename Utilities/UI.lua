...
local Debug = false
local MainAssetFolder = Debug and game.ReplicatedStorage["Bracket"]
or InsertService:LoadLocalAsset("rbxassetid://9912935003")
...
local function GetConfigs()
    ...
    if not isfolder("Hub\\Configs") then makefolder("Hub\\Configs") end
    if not isfile("Hub\\DefaultConfig.txt") then writefile("Hub\\DefaultConfig.txt","") end

    local Configs = {}
    for Index,File in pairs(listfiles("Hub\\Configs") or {}) do
        File = File:gsub("Hub\\Configs\\","")
        File = File:gsub(".json","")
        Configs[Index] = File
    end
    return Configs
...
local function ConfigsToList()
    ...
    for Index,File in pairs(listfiles("Hub\\Configs") or {}) do
        File = File:gsub("Hub\\Configs\\","")
        File = File:gsub(".json","")
        local DefaultConfig = readfile("Hub\\DefaultConfig.txt")
...
local function InitWindow(ScreenAsset,Window)
    ...
    function Window:SaveConfig(Name)
        ...
        if table.find(GetConfigs(),Name) then
            Config = HttpService:JSONDecode(readfile("Hub\\Configs\\"..Name..".json"))
        end
        ...
        writefile("Hub\\Configs\\"..Name..".json",HttpService:JSONEncode(Config))
    end
    function Window:LoadConfig(Name)
        if table.find(GetConfigs(),Name) then
            local DecodedJSON = HttpService:JSONDecode(readfile("Hub\\Configs\\"..Name..".json"))
            for Index,Element in pairs(Window.Elements) do
            ...
            local ConfigSection = Tab:Section({Name = "Configs",Side = Side}) do
                ...
                ConfigSection:Button({Name = "Set",Callback = function()
                    if ConfigDropdown.Value and ConfigDropdown.Value[1] then
                        ...
                        writefile("Hub\\DefaultConfig.txt",DefaultConfig)
                        ConfigDivider:SetText(
                        "Default Config\n<font color=\"rgb(189,189,189)\">[ "..DefaultConfig.." ]</font>")
                    end
                end})
                ConfigSection:Button({Name = "Clear",Callback = function()
                    writefile("Hub\\DefaultConfig.txt","")
