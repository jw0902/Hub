...
local Window = Hub.Utilities.UI:Window({
    Name = "Hub — "..Hub.Current,
    ...
    local GameTab = Window:Tab({Name = Hub.Current}) do
    ...
    local SettingsTab = Window:Tab({Name = "Settings"}) do
        ...
        Callback = Hub.Utilities.Misc.ReJoin})
        SettingsTab:Button({Name = "Server Hop",Side = "Left",
        Callback = Hub.Utilities.Misc.ServerHop})
        SettingsTab:Button({Name = "Join Discord Server",Side = "Left",
        Callback = Hub.Utilities.Misc.JoinDiscord})
...
local GetFPS = Hub.Utilities.Misc:SetupFPS()
--repeat task.wait() until Hub.Utilities.Drawing.Cursor
Hub.Utilities.Drawing:Cursor(Window.Flags)
Hub.Utilities.Drawing:FoVCircle("Aimbot",Window.Flags)
Hub.Utilities.Drawing:FoVCircle("Trigger",Window.Flags)
Hub.Utilities.Drawing:FoVCircle("SilentAim",Window.Flags)
    ...
    for Index,Config in pairs(getgc(true)) do
        ...
        and rawget(Config,"Model") then
            if Config.Recoil and Config.Recoil.Default and
                ...
                local Modified = Hub.Config.GameFeatures.WeaponModification
...
Notify.Event:Connect(function(Text)
    Hub.Utilities.UI:Notification2(Text)
end)
    ...
    if GunModel then
        for Index,Instance in pairs(GunModel.Body:GetDescendants()) do
            ...
            and Instance.Reflectance < 1 then
                Instance.Color = Hub.Utilities.UI:TableToColor(Config.Color)
    ...
    if Weapon and Config then
        ...
        if Ammo and Ammo.Server.Value > 0 then if not Hitbox then return end
            ...
            if (OldAmmo - Ammo.Server.Value) >= 1 then
                Hub.Utilities.UI:Notification2({
...
Hub.Utilities.Misc:NewThreadLoop(0,function()
...
Hub.Utilities.Misc:NewThreadLoop(1,function()
    local Weapon,Config = GetEquippedWeapon()
    if Weapon and Config then
...
for Index,Player in pairs(PlayerService:GetPlayers()) do
    if Player ~= LocalPlayer then
        Hub.Utilities.Drawing:AddESP(Player,"Player","ESP/Player",Window.Flags)
    end
end
...
PlayerService.PlayerRemoving:Connect(function(Player)
    Hub.Utilities.Drawing:RemoveESP(Player)
end)
